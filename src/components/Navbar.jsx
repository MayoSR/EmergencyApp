...
export default function Navbar() {
    ...
    return (
        <Flex position="fixed" bottom="0" left="0" right="0" width="100vw" boxShadow="-2px -3px 5px 0px rgba(0,0,0,0.75)" justifyContent="space-evenly" alignItems="center" bg="#22262a" height="60px" p={3}>
            <Icon color={selectedIcon === 1 ? "#589254" : "#f8f8f8"} filter={selectedIcon === 1 ? "drop-shadow(0px -2px 3px #589254)" : "drop-shadow(0px 0px 0px #fff)"} w={8} h={8} as={AiFillHome} onClick={() => selectTab(1, "home")} />
            <Icon color={selectedIcon === 2 ? "#589254" : "#f8f8f8"} filter={selectedIcon === 2 ? "drop-shadow(0px -2px 3px #589254)" : "drop-shadow(0px 0px 0px #fff)"} w={8} h={8} as={GiAmbulance} onClick={() => selectTab(2, "emergency")} />
            <Icon color={selectedIcon === 3 ? "#589254" : "#f8f8f8"} filter={selectedIcon === 3 ? "drop-shadow(0px -2px 3px #589254)" : "drop-shadow(0px 0px 0px #fff)"} w={8} h={8} as={BiLogOut} onClick={() => selectTab(3, "")} />
        </Flex>
    )
}
